...
(function () {
    ...
    let currentNick = '';
    const devSessionAuth = new Set(); // Запоминаем кто уже ввёл пароль в этой сессии
    ...
    function isDevSessionOk(nick) {
        return devSessionAuth.has(nick.toLowerCase().trim());
    }

    function markDevSession(nick) {
        devSessionAuth.add(nick.toLowerCase().trim());
    }
    ...
    function showNickModal() {
        ...
        if (lastNick) {
            // Если это дев и уже авторизован в сессии — сразу в игру
            if (isDevNick(lastNick) && isDevSessionOk(lastNick)) {
                currentNick = lastNick;
                hideNickModal();
                launchGame();
                return;
            }
    ...
    function handleNickInput(nick) {
        ...
        if (isDevNick(nick)) {
            // Если уже авторизован в сессии — показываем приветствие без пароля
            if (isDevSessionOk(nick)) {
                ...
                if (rec) {
                    nickWelcome.innerHTML = `
                        <p>С возвращением, <strong>${rec.nick}</strong>! ${role} 👋</p>
                        <p class="welcome-record">Твой рекорд: ${rec.score} очков (волна ${rec.wave})</p>`;
                } else {
                    nickWelcome.innerHTML = `<p>С возвращением, <strong>${nick}</strong>! ${role} 👋</p>`;
                }
                return;
            }
    ...
    const SOUND_FILES = {
        ...
        bossWarn: 'snd_boss_warn.mp3', bossKill: 'snd_boss_kill.mp3', gameover: 'snd_gameover.mp3',
    };
    ...
    function loadAllSounds() {
        ...
        entries.forEach(([key, url]) => {
            fetch(url).then(r => r.arrayBuffer()).then(buf => audioCtx.decodeAudioData(buf))
    ...
    function playGameoverSound() {
        ...
        try {
            ...
            setTimeout(() => {
                const s = audioCtx.createBufferSource(), g = audioCtx.createGain();
                s.buffer = soundBuffers['gameover']; g.gain.value = SOUND_VOLUME['gameover'] || 0.7;
                s.connect(g); g.connect(audioCtx.destination); s.start(0);
            }, 50);
        } catch (e) {}
    }
    ...
    function playSynthShoot() {
        if (!audioCtx) return; try { if (audioCtx.state === 'suspended') audioCtx.resume();
        ...
        o.connect(g); g.connect(audioCtx.destination); o.start(n); o.stop(n+0.1); } catch(e){}
    }
    ...
    function playSynthHit() {
        ...
        o.connect(g); g.connect(audioCtx.destination); o.start(n); o.stop(n+0.08); } catch(e){}
    }
    ...
    function playSynthWave() {
        ...
        o.connect(g); g.connect(audioCtx.destination); o.start(n); o.stop(n+0.4); } catch(e){}
    }

    function playSynthWaveDone() {
        if (!audioCtx) return; try { if (audioCtx.state === 'suspended') audioCtx.resume();
        const n = audioCtx.currentTime;
        [523,659,784].forEach((f,i) => { const o=audioCtx.createOscillator(),g=audioCtx.createGain(),t=n+i*0.12;
        o.type='sine'; o.frequency.setValueAtTime(f,t); g.gain.setValueAtTime(0.08,t);
        g.gain.exponentialRampToValueAtTime(0.001,t+0.3); o.connect(g); g.connect(audioCtx.destination);
        o.start(t); o.stop(t+0.3); }); } catch(e){}
    }

    function startMusic() {
        musicTrack.currentTime = 0; musicPlaying = false;
        function tryPlay() { musicTrack.play().then(() => { musicPlaying = true; })
            .catch(() => { if (!musicPlaying && running) setTimeout(tryPlay, 500); }); }
        tryPlay();
    }
    ...
    function unlockAudio() {
        if (audioUnlocked) return; audioUnlocked = true; initAudio();
        const v = musicTrack.volume; musicTrack.volume = 0.001;
        musicTrack.play().then(() => { musicTrack.pause(); musicTrack.currentTime = 0; musicTrack.volume = v; })
    ...
    const CFG = {
        ...
        enemyBaseHp:1, enemyHpGrow:0.15, enemySpeed:1.0, enemySpeedGrow:0.08,
    ...
    function spawnEnemy(isBoss){
        ...
        enemies.push({x:Math.random()*(W-sz*2)+sz,y:-sz-30,w:sz,h:sz,speed:spd,hp,maxHp:hp,
            img:isBoss?BOSS_IMG:ENEMY_IMGS[Math.floor(Math.random()*ENEMY_IMGS.length)],
            boss:!!isBoss,zigzag:!isBoss&&Math.random()>0.4,zigAmp:(Math.random()-0.5)*4,
            angle:Math.random()*Math.PI*2,flash:0,bossDir:Math.random()>0.5?1:-1,
            bossDiveTimer:0,bossDiving:false,bossReturning:false});
    }
    function isOnScreen(e){return(e.y-e.h/2)>=0;}
    function updateEnemies(dt){
        for(let i=enemies.length-1;i>=0;i--){const e=enemies[i];
        if(!e.boss){e.y+=e.speed;if(e.zigzag){e.angle+=0.04;e.x+=Math.sin(e.angle)*e.zigAmp;}
        e.x=Math.max(e.w/2,Math.min(W-e.w/2,e.x));if(e.y>H+e.h){enemies.splice(i,1);playerHit();continue;}}
        if(e.boss){if(!isOnScreen(e)&&!e.bossDiving){e.y+=1.5;e.x=Math.max(e.w/2,Math.min(W-e.w/2,e.x));if(e.flash>0)e.flash-=dt;continue;}
        ...
        if(!e.bossDiving&&!e.bossReturning){e.bossDiveTimer+=dt;if(e.bossDiveTimer>Math.max(1800,4000-wave*200)){e.bossDiving=true;e.bossDiveTimer=0;}
        const ty=H*0.15+e.h/2;if(e.y<ty-3)e.y+=1;else if(e.y>ty+3)e.y-=1;e.y+=Math.sin(Date.now()/800)*0.4;}
        if(e.bossDiving){e.y+=5+wave*0.4;if(e.y>=player.y-10){e.y=player.y-10;e.bossDiving=false;e.bossReturning=true;doShake(6,200);}
        ...
        e.x=Math.max(e.w/2,Math.min(W-e.w/2,e.x));}if(e.flash>0)e.flash-=dt;}
    }
    function drawEnemies(){
        for(const e of enemies){if(!e.boss&&!isOnScreen(e))continue;ctx.save();
        if(e.flash>0){ctx.shadowBlur=25;ctx.shadowColor='#fff';}
        ctx.drawImage(e.img,e.x-e.w/2,e.y-e.h/2,e.w,e.h);
        if(e.maxHp>1&&e.hp>0){const bw=e.w*(e.boss?1.3:1),bh=e.boss?8:5,bx=e.x-bw/2,by=e.y-e.h/2-(e.boss?20:12),r=e.hp/e.maxHp;
        ...
        if(e.boss&&e.hp>0){ctx.fillStyle='#ff0';ctx.font='bold 14px Orbitron';ctx.textAlign='center';
        ctx.shadowBlur=8;ctx.shadowColor='#ff0';ctx.fillText('ГАЛЮХА-БОСС',e.x,e.y-e.h/2-28);}ctx.restore();}
    }
    ...
    function checkCollisions(){
        ...
        if(e.hp<=0){const pts=e.boss?500*wave:100;score+=pts;elScore.textContent=score;
        boom(e.x,e.y,e.boss?55:20,e.boss?'#ff0':'#f80');addText(e.x,e.y,'+'+pts,e.boss?'#ff0':'#0ff');
        if(e.boss){bossAlive=false;doShake(16,600);addText(W/2,H/2,'ГАЛЮХА УНИЧТОЖЕНА!','#0f0',26,0.008);playSound('bossKill');}
        else playSound('kill');enemies.splice(ei,1);}break;}}}
    ...
    function gameOver(){
        running=false;cancelAnimationFrame(animId);stopMusic();setTimeout(()=>playGameoverSound(),100);
    ...
    function startWave(){
        waveState='active';const isBoss=wave%CFG.bossEvery===0;
        if(isBoss){toSpawn=0;spawnEnemy(true);bossAlive=true;addText(W/2,H/2-30,'⚠ БОСС-ГАЛЮХА ⚠','#ff0',28,0.008);playSound('bossWarn');}
        else{toSpawn=CFG.enemyCount+wave*2;spawnInterval=Math.max(350,CFG.spawnDelay-wave*40);spawnTimer=0;addText(W/2,H/2,'ВОЛНА '+wave,'#0ff',32,0.008);playSynthWave();}
        elWave.textContent=wave;
    }
    function updateWave(dt){
        if(waveState==='active'){if(toSpawn>0){spawnTimer+=dt;if(spawnTimer>=spawnInterval){spawnTimer=0;spawnEnemy(false);toSpawn--;}}
        ...
        else{wavePauseTimer+=dt;if(wavePauseTimer>=CFG.wavePause){wave++;startWave();}}
    }
    ...
    function loop(time){
        ...
        updateParticles();updateTexts();checkCollisions();updateWave(dt);updateShake(dt);
        ctx.save();ctx.translate(shakeX,shakeY);ctx.fillStyle='#050510';ctx.fillRect(-10,-10,W+20,H+20);
        drawStars();drawBullets();drawEnemies();drawPlayer();drawParticles();drawTexts();ctx.restore();
    }
    ...
    function launchGame(){
        ...
        running=true;lastTime=performance.now();animId=requestAnimationFrame(loop);startWave();},200);
    }
