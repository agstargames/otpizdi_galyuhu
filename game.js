...
(function () {
    ...
    function getPlayerRecord(nick) {
        return getRecords().find(r => r.nick.toLowerCase() === nick.toLowerCase()) || null;
    }
    ...
    function showNickModal() {
        ...
        if (lastNick) {
            if (isDevNick(lastNick) && isDevSessionOk(lastNick)) {
                currentNick = lastNick; hideNickModal(); startGameDirect(); return;
            }
    ...
    function handleNickInput(nick) {
        ...
        if (isDevNick(nick)) {
            ...
            setTimeout(() => {
                const pi = getPassInput();
                if (pi) pi.addEventListener('keydown', e => { if (e.key === 'Enter') btnNickGo.click(); });
            }, 50);
        } else {
            const rec = getPlayerRecord(nick);
            if (rec) {
    ...
    nickInput.addEventListener('keydown', e => {
        if (e.key === 'Enter') {
            const pi = getPassInput();
            if (pi && !pi.value) pi.focus(); else btnNickGo.click();
        }
    });
    ...
    function loadAllSounds() {
        const entries = Object.entries(SOUND_FILES); let loaded = 0;
        entries.forEach(([key, url]) => {
            fetch(url).then(r => r.arrayBuffer())
                .then(buf => audioCtx.decodeAudioData(buf))
                .then(decoded => { soundBuffers[key] = decoded; loaded++; if (loaded === entries.length) audioReady = true; })
                .catch(() => { loaded++; if (loaded === entries.length) audioReady = true; });
        });
    }
    ...
    function playGameoverSound() {
        ...
        try {
            ...
            setTimeout(() => {
                const s = audioCtx.createBufferSource(), g = audioCtx.createGain();
                s.buffer = soundBuffers['gameover']; g.gain.value = 0.7;
                s.connect(g); g.connect(audioCtx.destination); s.start(0);
            }, 50);
        } catch (e) {}
    }
    function playSynthShoot() {
        ...
        try {
            ...
            o.frequency.exponentialRampToValueAtTime(220, n + 0.08);
    ...
    function playSynthWave() {
        ...
        try {
            if (audioCtx.state === 'suspended') audioCtx.resume();
            const o = audioCtx.createOscillator(), g = audioCtx.createGain(), n = audioCtx.currentTime;
            o.type = 'sine'; o.frequency.setValueAtTime(440, n);
            o.frequency.exponentialRampToValueAtTime(880, n + 0.2);
            g.gain.setValueAtTime(0.1, n); g.gain.linearRampToValueAtTime(0.1, n + 0.15);
    ...
    function unlockAudio() {
        ...
        musicTrack.play().then(() => {
            musicTrack.pause(); musicTrack.currentTime = 0; musicTrack.volume = v;
        }).catch(() => { musicTrack.volume = v; });
        if (audioCtx && audioCtx.state === 'suspended') audioCtx.resume();
    }
    ...
    // ============================================================
    //  ★ КОНФИГ + СИСТЕМА БАЛАНСА
    // ============================================================
    //  Всё масштабируется через smooth() — асимптотическая кривая.
    //  Параметры быстро растут на ранних волнах, потом выходят на плато.
    //  Игра ВСЕГДА проходима, просто становится сложнее.
    // ============================================================

    const CFG = {
        playerSpeed:  8,        // Скорость игрока (px за кадр при 60fps)
        bulletSpeed:  14,       // Скорость пуль
        fireRate:     130,      // Интервал стрельбы (мс)
        lives:        3,        // Стартовые жизни
        invincTime:   2000,     // Неуязвимость после удара (мс)
        wavePause:    2500,     // Пауза между волнами (мс)
        bossEvery:    5,        // Босс каждые N волн
        maxEnemyHp:   5         // Потолок HP обычного врага
    };

    // ─── Плавное масштабирование ───────────────────────────
    // При wave=1 возвращает startVal.
    // С ростом волн плавно приближается к endVal, но НИКОГДА его не превышает.
    // halfWave = волна, на которой значение проходит половину пути.
    //
    //  Примеры (smooth(1.5, 3.2, wave, 20)):
    //    wave  1 → 1.50    wave 10 → 2.03    wave 20 → 2.33
    //    wave 50 → 2.71    wave100 → 2.97    wave∞  → 3.20
    // ───────────────────────────────────────────────────────
    ...
    function updatePlayer(dt) {
        ...
        if (pointerX !== null) player.x += (pointerX - player.x) * (0.14 * s60);
    ...
    function drawPlayer() {
        ...
        ctx.beginPath(); ctx.moveTo(0, -25); ctx.lineTo(-20, 22); ctx.lineTo(-8, 16);
        ctx.lineTo(0, 22); ctx.lineTo(8, 16); ctx.lineTo(20, 22); ctx.closePath(); ctx.fill();
        ctx.fillStyle = '#0ff';
    ...
    function handleFiring(dt) {
        ...
        fireTimer -= dt;
        if (fireTimer <= 0) { shoot(); fireTimer = CFG.fireRate; }
    }
    function updateBullets(dt) {
        const s60 = dt / 16.666;
        const speed = CFG.bulletSpeed * s60;
    ...
    function drawBullets() {
        ctx.save(); ctx.shadowBlur = 10; ctx.shadowColor = '#0ff'; ctx.fillStyle = '#0ff';
    ...
    // ============================================================
    //  ★ ВРАГИ — спавн с балансным масштабированием
    // ============================================================
    function spawnEnemy(isBoss) {
        const sz = isBoss ? 110 : 48 + Math.random() * 24;

        // ── Скорость (только для обычных) ──
        // wave 1: ~1.5-2.1  |  wave 20: ~2.3-2.9  |  wave 50+: ~2.7-3.3
        // Босс НЕ использует e.speed (у него своя логика движения)
        const spd = isBoss
            ? 0
            : smooth(1.5, 3.2, wave, 20) + Math.random() * 0.6;

        // ── HP ──
        // Обычные: 1 → 2(w8) → 3(w16) → 4(w30) → 5(w75+)
        // Босс:    18 → плавно до ~60
        let hp;
        if (isBoss) {
            hp = Math.floor(smooth(18, 60, wave, 30));
        } else {
            hp = Math.min(CFG.maxEnemyHp, 1 + Math.floor(smooth(0, 5, wave, 15)));
        }

        const img = isBoss
            ? BOSS_IMG
            : (ENEMY_IMGS.length > 0 ? ENEMY_IMGS[Math.floor(Math.random() * ENEMY_IMGS.length)] : null);
    ...
    // ============================================================
    //  ★ ВРАГИ — обновление с плавным масштабированием
    // ============================================================
    function updateEnemies(dt) {
        ...
        for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (e.boss) {
                ...
                // ── Горизонтальная скорость босса ──
                // wave 5: ~2.85  |  wave 20: ~3.66  |  wave 50: ~4.36  |  max: 5.5
                const bossHorizSpd = smooth(2.5, 5.5, wave, 30) * s60;
                e.x += e.bossDir * bossHorizSpd;
                if (e.x < e.w / 2 + 20) { e.x = e.w / 2 + 20; e.bossDir = 1; }
                if (e.x > W - e.w / 2 - 20) { e.x = W - e.w / 2 - 20; e.bossDir = -1; }

                // ── Патруль (ожидание нырка) ──
                if (!e.bossDiving && !e.bossReturning) {
                    e.bossDiveTimer += dt;

                    // Интервал между нырками:
                    // wave 5: ~4117ms  |  wave 20: ~3379ms  |  wave 50: ~2866ms  |  min: 2200ms
                    const diveInterval = smooth(4500, 2200, wave, 20);
                    if (e.bossDiveTimer > diveInterval) {
                        e.bossDiving = true;
                        e.bossDiveTimer = 0;
                    }

                    // Плавно подходит к своей «высоте патруля»
                    const ty = H * 0.15 + e.h / 2;
                    if (e.y < ty - 3) e.y += 1 * s60;
                    else if (e.y > ty + 3) e.y -= 1 * s60;
                    e.y += Math.sin(Date.now() / 800) * 0.4 * s60;
                }

                // ── Нырок вниз ──
                if (e.bossDiving) {
                    // Скорость нырка:
                    // wave 5: ~5.86  |  wave 20: ~6.78  |  wave 50: ~7.77  |  max: 9.0
                    const diveSpd = smooth(5.5, 9.0, wave, 35) * s60;
                    e.y += diveSpd;
                    if (e.y >= player.y - 10) {
                        e.y = player.y - 10;
                        e.bossDiving = false; e.bossReturning = true;
                        doShake(6, 200);
                    }
                    if (e.y > H - 30) {
                        e.y = H - 30;
                ...
                // ── Возврат наверх ──
                if (e.bossReturning) {
                    // Возврат чуть быстрее чем нырок, чтобы босс не зависал внизу
                    const retSpd = smooth(4.5, 7.0, wave, 25) * s60;
                    e.y -= retSpd;
                    if (e.y <= H * 0.15 + e.h / 2) {
                        e.y = H * 0.15 + e.h / 2;
    ...
    function drawEnemies() {
        for (const e of enemies) {
            ...
            if (e.boss && e.hp > 0) {
                ctx.fillStyle = '#ff0'; ctx.font = 'bold 14px Orbitron';
                ctx.textAlign = 'center'; ctx.shadowBlur = 8; ctx.shadowColor = '#ff0';
                ctx.fillText('ГАЛЮХА-БОСС', e.x, e.y - e.h / 2 - 28);
            }
            ctx.restore();
        }
    }
    ...
    // ===== ЧАСТИЦЫ + ТЕКСТ =====
    function boom(x, y, c, col) {
        for (let i = 0; i < c; i++) {
            const a = Math.random() * Math.PI * 2, v = Math.random() * 5 + 2;
            particles.push({
                x, y, vx: Math.cos(a) * v, vy: Math.sin(a) * v,
                r: Math.random() * 4 + 1.5, life: 1,
                decay: Math.random() * 0.025 + 0.015,
                color: col || `hsl(${Math.random() * 50 + 10},100%,55%)`
            });
        }
    }
    function updateParticles(dt) {
        ...
        for (let i = particles.length - 1; i >= 0; i--) {
            ...
            p.vy += 0.06 * s60; p.life -= p.decay * s60;
    ...
    function addText(x, y, t, c, s, d) {
        floatTexts.push({ x, y, text: t, color: c, size: s || 22, life: 1, vy: -1.2, decay: d || 0.015 });
    }
    function updateTexts(dt) {
        const s60 = dt / 16.666;
        for (let i = floatTexts.length - 1; i >= 0; i--) {
            const t = floatTexts[i];
            t.y += t.vy * s60; t.life -= t.decay * s60;
    ...
    function spawnHeart() {
        hearts.push({
            ...
            size: 22, speed: 1.5 + Math.random() * 0.8, glow: 0
        });
    }
    function updateHearts(dt) {
        ...
        if (heartTimer >= HEART_INTERVAL) {
            heartTimer = 0;
            if (Math.random() < HEART_CHANCE && lives < MAX_LIVES) spawnHeart();
        }
        for (let i = hearts.length - 1; i >= 0; i--) {
            const h = hearts[i];
            h.y += h.speed * s60; h.glow += 0.05 * s60;
            if (h.y > H + 40) { hearts.splice(i, 1); continue; }
            if (dist(player.x, player.y, h.x, h.y) < 30) {
                hearts.splice(i, 1);
                if (lives < MAX_LIVES) {
                    lives++; elLives.textContent = lives;
                    addText(h.x, h.y, '+1 ❤️', '#ff4444', 24);
                    boom(h.x, h.y, 12, '#ff4444'); playSynthHeal();
                }
            }
        }
    }
    function drawHearts() {
        for (const h of hearts) {
            ...
            ctx.shadowBlur = 15 + Math.sin(h.glow * 2) * 5;
            ctx.shadowColor = '#ff4444'; ctx.fillStyle = '#ff4444';
            ctx.beginPath(); ctx.moveTo(0, -8);
            ctx.bezierCurveTo(-12, -22, -26, -10, -14, 4);
            ctx.lineTo(0, 18); ctx.lineTo(14, 4);
            ctx.bezierCurveTo(26, -10, 12, -22, 0, -8); ctx.fill();
            ctx.fillStyle = 'rgba(255,255,255,0.35)';
            ctx.beginPath(); ctx.ellipse(-6, -10, 4, 3, -0.5, 0, Math.PI * 2); ctx.fill();
            ctx.restore();
        }
    }
        ...
        for (let bi = bullets.length - 1; bi >= 0; bi--) {
            ...
            for (let ei = enemies.length - 1; ei >= 0; ei--) {
                ...
                if (dist(b.x, b.y, e.x, e.y) < e.w / 2 + 6) {
                    bullets.splice(bi, 1); e.hp--; e.flash = 80;
                    boom(b.x, b.y, 4, '#0ff'); playSynthHit();
                    if (e.hp <= 0) {
    ...
    function playerHit() {
        ...
        lives--; elLives.textContent = lives;
        invincible = true; invTimer = CFG.invincTime;
        doShake(8, 250); playSound('playerHit');
        if (lives <= 0) gameOver();
    }
    function gameOver() {
        ...
        if (isNew) elNewRecord.classList.remove('hidden');
    ...
    // ============================================================
    //  ★ ВОЛНЫ — с плавным масштабированием
    // ============================================================
    function startWave() {
        waveState = 'active';
        ...
        if (isBoss) {
            ...
            // wave 1: 5  |  wave 10: 9  |  wave 20: 12  |  wave 50: 16  |  max: ~22
            toSpawn = Math.floor(smooth(5, 22, wave, 25));

            // ── Интервал спавна (мс) ──
            // wave 1: 1100ms  |  wave 10: 856ms  |  wave 20: 620ms  |  wave 50: 530ms  |  min: ~450ms
            spawnInterval = smooth(1100, 450, wave, 15);
        ...
        if (waveState === 'active') {
            if (toSpawn > 0) {
                ...
                if (spawnTimer >= spawnInterval) {
                    spawnTimer = 0; spawnEnemy(false); toSpawn--;
                }
            }
            if (toSpawn <= 0 && enemies.length === 0) {
                waveState = 'cooldown'; wavePauseTimer = 0;
    ...
    function loop(time) {
        ...
        drawStars(); drawBullets(); drawEnemies(); drawHearts();
    ...
    function startGameDirect() {
        ...
        hearts = []; heartTimer = 0;
        invincible = false; invTimer = 0; bossAlive = false;
    ...
    btnMenu.addEventListener('click', () => {
        running = false; cancelAnimationFrame(animId); stopMusic(); showScreen(menuScreen);
    });
    ...
    canvas.addEventListener('touchend', e => {
        ...
        else { pointerX = null; touchActive = false; firing = false; }
    }, { passive: false });
